  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      await hydraWriteMemory({
        ...
        metadata: { eventId: data.id, provider: f.provider, tags },
      });
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">A visit, lab, medication, or anything worth remembering.</p>
      </div>
      ...
      <form onSubmit={submit} className="space-y-5 rounded-2xl border border-border bg-card p-6 shadow-soft">
        ...
        <div className="space-y-1.5">
          <Label>Title</Label>
          <Input value={f.title} onChange={(e) => setF({ ...f, title: e.target.value })} placeholder="e.g. Annual physical" required />
        </div>
        <div className="space-y-1.5">
          <Label>Notes</Label>
          <Textarea rows={4} value={f.description} onChange={(e) => setF({ ...f, description: e.target.value })} placeholder="Findings, instructions, follow-ups…" />
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5">
            <Label>Provider / clinician</Label>
            <Input value={f.provider} onChange={(e) => setF({ ...f, provider: e.target.value })} placeholder="Dr. Patel" />
          </div>
          <div className="space-y-1.5">
            <Label>Location</Label>
            <Input value={f.location} onChange={(e) => setF({ ...f, location: e.target.value })} placeholder="Clinic, hospital" />
          </div>
        </div>
        <div className="space-y-1.5">
          <Label>Tags (comma separated)</Label>
          <Input value={f.tags} onChange={(e) => setF({ ...f, tags: e.target.value })} placeholder="cardiology, follow-up" />
        </div>
